...
export const Config: Schema<Config> = Schema.object({
  ...
  撤回消息:Schema.boolean()
    .description("是否撤回触发关键词的消息（机器人需要是管理员）"),
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.guild().middleware(async (session, next) => {
    ...
    for (let i of (data.length === 0 ? config.全局提醒关键词 : config.全局提醒关键词.concat(data[0].keywords))) {
      if (new RegExp(`${i}`).test(session.content) && (config.自我触发 ? true : !config.超级管理员.includes(session.event.user.id))) {
        result.push(i)
      }
    }
    if (result.length > 0) {
      if (config.撤回消息) {
        await session.bot.deleteMessage(session.event.channel.id, session.event.message.id)
      }
      for (let admin of config.超级管理员) {
        const channel = await session.bot.createDirectChannel(admin)
